 ...
 */
package com.alibaba.csp.sentinel.dashboard.dashboard.discovery;
...
public class AppInfo {
    ...
    private Set<com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo> machines = ConcurrentHashMap.newKeySet();
     ...
     */
    public Set<com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo> getMachines() {
    ...
    public boolean addMachine(com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo machineInfo) {
        machines.remove(machineInfo);
        return machines.add(machineInfo);
    }

    public synchronized boolean removeMachine(String ip, int port) {
        Iterator<com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo> it = machines.iterator();
        while (it.hasNext()) {
            com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo machine = it.next();
    ...
    public Optional<com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo> getMachine(String ip, int port) {
    ...
    private boolean heartbeatJudge(final int threshold) {
        ...
        if (threshold > 0) {
            long healthyCount = machines.stream()
                .filter(com.alibaba.csp.sentinel.dashboard.discovery.MachineInfo::isHealthy)
                .count();
            if (healthyCount == 0) {
                // No healthy machines.
                return machines.stream()
